...
以下分析方向依托已采集数据，识别可信度较高的因果识别策略，按可行性排序：

### ⭐ 优先级 1：野火 × WQP（最干净的自然实验）

| 维度 | 内容 |
|------|------|
| **Treatment** | 野火发生时间 × 地点（`fires.geojson` 含精确边界和时间戳） |
| **Outcome** | WQP 监测站月均污染物浓度（重金属、浊度、硝酸盐） |
| **识别设计** | DiD — 受灾区域下游站点 vs 上游/对照站点，火前 vs 火后 |
| **为什么可信** | 野火发生时间基本外生；机制清晰（灰烬径流带入重金属/氮）；有月度面板 |
| **数据就绪** | ✅ fires.geojson + WQP stations/results.csv + NOAA 气候控制变量 |
| **下一步** | 按流域或直线距离确定 WQP 站点上下游关系，构造处理组/对照组 |

### ⭐ 优先级 2：NPDES 废水排放 → 下游 WQP 水质（空间回归）

| 维度 | 内容 |
|------|------|
| **Treatment** | NPDES 设施月均排放量（MGD，已通过 EPA ECHO effluent chart API 获取） |
| **Outcome** | 下游方向 WQP 站点同月的污染物浓度（电导率、氨氮、重金属） |
| **识别设计** | 空间滞后模型；按排放量变化月份构造准 DiD |
| **数据就绪** | ✅ npdes.geojson + npdes_timeseries.json（15 个设施月流量）+ WQP results |
| **挑战** | 需要确定上下游关系（LA County 河流流向数据） |

### ⭐ 优先级 3：CDPR 农药使用 → 地下水污染（空间匹配）

| 维度 | 内容 |
|------|------|
| **Treatment** | COMTRS 地块农药施用量（磅/年，pesticide.geojson 已有 132 个地块）|
| **Outcome** | 附近 DWR 地下水井的硝酸盐/农药残留浓度 |
| **识别设计** | 空间匹配：每个地下水井匹配最近农药地块；缓冲区回归（0-1km / 1-3km / >3km）|
| **数据就绪** | ✅ pesticide.geojson + groundwater.geojson |
| **优势** | 不同作物用药量差异大，提供良好的处理变量变异 |

### 优先级 4：污染暴露 → 健康结果（EJScreen × CDC PLACES）

| 维度 | 内容 |
|------|------|
| **Treatment** | ZCTA/Tract 级别水质污染指数（WQP 站点插值）或空气质量（AQS）|
| **Outcome** | CDC PLACES 健康结果（哮喘率、癌症率、肾病率，Tract 粒度）|
| **识别设计** | 截面空间回归 + 工具变量（历史工业区划作为 IV） |
| **数据就绪** | ✅ ejscreen.geojson + cdc_places.geojson + aqs.geojson |
| **挑战** | 截面数据无法排除所有混淆；需强 IV |

### 优先级 5：学校铅含量 × 建筑年代（自然断点）

| 维度 | 内容 |
|------|------|
| **Treatment** | 供水管网建造年代（1986年《安全饮用水法》修订为铅管禁用分界线）|
| **Outcome** | 学校铅水采样浓度（DDW，5,979 条记录）|
| **识别设计** | RD（回归断点）——以 1986 年为阈值，比较前后建造学校铅浓度差异 |
| **数据就绪** | ✅ schools.geojson + Census 1970前建房比例作为代理变量 |
